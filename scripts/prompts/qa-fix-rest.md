# Agent task: fix agent — REST API surface (Phase 14)
...
You are a fix agent for the REST API QA surface. You read findings from the QA agent, apply fixes
...
1. **Read findings** — read `/tmp/qa-findings-rest.md`. Only act on findings with `Status: FOUND`.
...
2. **Fix each FOUND finding** — for each `Status: FOUND` finding:
   ...
   - Apply the minimal fix in `crates/triad-runner/src/admin.rs` (or related source) that
     aligns the implementation with `CLAUDE.md` admin endpoints table and `triad-system-design.md`.
...
4. **Set `State: ALL_FIXED`** in `/tmp/qa-findings-rest.md` so the QA agent can re-verify.

5. **Commit** to `feat/qa-fixes`:
   ```bash
   git -C /home/jreuben1/Code/triad-worktrees/qa-fixes add -p
   git -C /home/jreuben1/Code/triad-worktrees/qa-fixes commit -m "fix(rest): <finding titles>"
   ```
...
7. **Publish event**:
   ```bash
   printf '{"ts":"%s","agent":"qa-fixes","phase":14,"event":"step_done","detail":"REST fixes committed: <N> findings fixed","coverage_pct":null}\n' "$(date -Iseconds)" >> /tmp/triad-agent-events.jsonl
   ```
...
- [ ] All `Status: FOUND` REST findings changed to `Status: FIXED`
- [ ] `/tmp/qa-findings-rest.md` `State: ALL_FIXED`
