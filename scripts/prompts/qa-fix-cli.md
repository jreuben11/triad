# Agent task: fix agent — CLI surface (Phase 14)
...
You are a fix agent for the CLI QA surface. You read findings from the QA agent, apply fixes
...
1. **Read findings** — read `/tmp/qa-findings-cli.md`. Only act on findings with `Status: FOUND`.
...
2. **Fix each FOUND finding** — for each `Status: FOUND` finding:
   ...
   - Apply the minimal fix in `crates/` that aligns the implementation with the design doc.
...
4. **Set `State: ALL_FIXED`** in `/tmp/qa-findings-cli.md` so the QA agent can re-verify.

5. **Commit** to `feat/qa-fixes`:
   ```bash
   git -C /home/jreuben1/Code/triad-worktrees/qa-fixes add -p
   git -C /home/jreuben1/Code/triad-worktrees/qa-fixes commit -m "fix(cli): <finding titles>"
   ```
...
7. **Publish event**:
   ```bash
   printf '{"ts":"%s","agent":"qa-fixes","phase":14,"event":"step_done","detail":"CLI fixes committed: <N> findings fixed","coverage_pct":null}\n' "$(date -Iseconds)" >> /tmp/triad-agent-events.jsonl
   ```

## Open a PR when all four surfaces are PASSED

Only after ALL four findings files show `State: PASSED`:
...
- [ ] All `Status: FOUND` CLI findings changed to `Status: FIXED`
- [ ] `/tmp/qa-findings-cli.md` `State: ALL_FIXED`
