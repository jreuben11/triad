# Agent task: fix agent — TUI surface (Phase 14)
...
You are a fix agent for the TUI QA surface. You read findings from the QA agent, apply fixes
to the codebase, run the quality gate, and commit. You do NOT run in `/loop` — you are
...
1. **Read findings** — read `/tmp/qa-findings-tui.md`. Only act on findings with `Status: FOUND`.
...
2. **Fix each FOUND finding** — for each `Status: FOUND` finding:
   ...
   - Apply the minimal fix in `crates/triad-tui/src/` that aligns the implementation with
     `triad-system-design.md` and the screen specs in `scripts/prompts/triad-tui.md`.
   - Common TUI fix areas: `app/mod.rs` (state machine), `app/input.rs` (key handling),
     individual screen files, layout constraints causing overflow.
...
4. **Set `State: ALL_FIXED`** in `/tmp/qa-findings-tui.md` so the QA agent can re-verify.

5. **Commit** to `feat/qa-fixes`:
   ```bash
   git -C /home/jreuben1/Code/triad-worktrees/qa-fixes add -p
   git -C /home/jreuben1/Code/triad-worktrees/qa-fixes commit -m "fix(tui): <finding titles>"
   ```
...
7. **Publish event**:
   ```bash
   printf '{"ts":"%s","agent":"qa-fixes","phase":14,"event":"step_done","detail":"TUI fixes committed: <N> findings fixed","coverage_pct":null}\n' "$(date -Iseconds)" >> /tmp/triad-agent-events.jsonl
   ```
...
Update `project-plan.md` Phase 14 items and `claude-best-practices-learned.md` with any
QA patterns worth recording. Commit all three files together.

## Done criteria

- [ ] All `Status: FOUND` TUI findings changed to `Status: FIXED`
- [ ] `/tmp/qa-findings-tui.md` `State: ALL_FIXED`
- [ ] Quality gate passing on `feat/qa-fixes`
